...
config = {'username': '', 'client_id': '', 'redirect_url': '', 'secret_key': '', 'totp_key': '',
          ...
          'token generated_on': '', 'stop_loss_percent': ''
          }
...
class Buy_Order:
    def __init__(self) -> None:
        global config
        try:
            if not os.path.join(os.getcwd().join('Buy_config.json'.strip())):
        ...
        self.config = config
        self.model = None
        if self.username == '' or self.client_id == '' or self.secret_key == '' or self.totp_key == '' or self.redirect_url == '' or self.target1 == '' or self.target2 == '' or self.stop_loss1 == '' or self.stop_loss2 == '' or self.quantity1 == '' or self.quantity2 == '' or self.stop_loss_percent == '':
    ...
    def create_config_file(self):
        ...
        with open("../config/Buy_config.json", "w") as f:
            f.write(file)
            f.close()
    ...
    def get_model(self):
        if self.model is None:
            ...
            self.model = fyersModel.FyersModel(client_id=self.client_id, token=self.access_token,
                                               log_path="C:\\Users\\ADMIN\\Documents\\Fyers_Logs")
            return self.model
    ...
    def place_order_1(self):
        ...
        try:
            while True:
                ...
                time.sleep(2)
                config4 = json.load(open('Buy_config.json'))
                ...
                if str(ltp) <= config4['limitPrice1']:
        ...
        while intrade == 1:
            ...
            config1 = json.load(open('Buy_config.json'))
            ExitId = {"id": "" + (self.symbol + '-' + self.productType) + ''}
            entry_price = config1['limitPrice1']
            stop_loss_percent = 2
            current_price = ltp
            trailing_stop_loss_price = dataconfig.trailing_stop_loss(current_price, int(entry_price), stop_loss_percent)

            if time.strftime('%H:%M') == '15:12':
                dataconfig.exit_position(ExitId)
                print('Trade Squre OFF')
                return
            print('self.stop_loss1', trailing_stop_loss_price)
            print('self.target1', config1['target1'])
            if (side == 1) and (ltp <= trailing_stop_loss_price):
                ...
                while intrade == 0:
                    ...
                    if time.strftime('%H:%M') == '15:12':
                        dataconfig.exit_position(ExitId)
                        print('Trade Square OFF')
                        return
                    print('self.stop_loss2', config1['stop_loss2'])
                    print('self.target2', config1['target2'])
    ...
    def place_order_2(self):
        ...
        try:
            ...
            while True:
                ...
                time.sleep(2)
                config4 = json.load(open('Buy_config.json'))
                print('Order still not placed in place_order 2 for ', self.symbol, 'with LTP -', ltp,
    ...
    def trailing_stop_loss(self, current_price, entry_price, stop_loss_percent):
        """
        Calculates the trailing stop loss price based on the current price, entry price,
        and the stop loss percent.
        """
        stop_loss_price = entry_price - (entry_price * stop_loss_percent / 100)
        if current_price > entry_price:
            # Update stop loss price if current price is higher than entry price
            stop_loss_price = max(stop_loss_price, current_price - (current_price * stop_loss_percent / 100))
