...
config = {'username': '', 'client_id': '', 'redirect_url': '', 'secret_key': '', 'totp_key': '',
          ...
          'token generated_on': '', 'stoploss_count': ''
          }
...
class Sell_Order:
    ...
    def __init__(self) -> None:
        ...
        global config
        try:
            if not os.path.join(os.getcwd().join('Sell_config.json'.strip())):
        ...
        config = json.load(open('Sell_config.json'))
        # threading.Thread(target=Sell_Order.fileWatcher).start()
        print('sell_config--', config)
        ...
        self.stopLoss_count = config['stopLoss_count']
        self.config = config
        self.model = None
        if self.username == '' or self.client_id == '' or self.secret_key == '' or self.totp_key == '' or self.redirect_url == '' or self.target1 == '' or self.target2 == '' or self.stop_loss1 == '' or self.stop_loss2 == '' or self.quantity1 == '' or self.quantity2 == '' or self.stopLoss_count == '':
            print('self.username-', self.username, self.client_id, self.secret_key, self.totp_key, self.redirect_url,
                  ...
                  'self.stopLoss_count', self.stopLoss_count)
            print('please enter all the details in the Sell_config.json file')
            exit()

    def create_config_file(self):
        file = json.dumps(config, indent=10)
        with open("../config/Sell_config.json", "w") as f:
            f.write(file)
            f.close()
    ...
    def generate_token(self, refresh=False):
        ...
        try:
            ...
            with open('Sell_config.json', 'w') as f:
    ...
    def place_order_1(self, stopLossList):

        Stoploss_Count = 0
        if stopLossList is None:
            stopLossList = []
        side = -1
        intrade = 1
        ...
        try:
            ...
            while Flag:
                ...
                print('current time--', time.strftime("\033[96mBold %H:%M:%S"))
                time.sleep(1)
                config5 = json.load(open('Sell_config.json'))
                print('Order still not placed in place_order 1 for ', self.symbol, 'with LTP -', ltp,
                      ' and with LIMIT Price1 - ',
                      config5['limitPrice1'])
                if str(ltp) >= config5['limitPrice1']:
                    response = fyers.place_order(data1)
                    print('Order Placed  in place_order 1 with LTP ', ltp, response)
        ...
        while intrade == 1:
            ...
            time.sleep(1)
            config1 = json.load(open('Sell_config.json'))
            ExitId = {"id": "" + (self.symbol + '-' + self.productType) + ''}
            if time.strftime('%H:%M') == '15:12':
                dataconfig.exit_position(ExitId)
                print('Trade is Squared OFF')
                return

            print('Stop_loss1 value is - ', config1['stop_loss1'])
            print('Target1 value is - ', config1['target1'])
            if (side == -1) and (str(ltp) >= config1['stop_loss1']):
                print("Stop loss hit first time of " + self.symbol + ' at ' + str(ltp))
                # ExitId = {"id": "NSE:AARTIIND-EQ-INTRADAY"}
                print('exit data', ExitId)
                stopLossList.append(Stoploss_Count + 1)
                print('Numbers of times StopLoss hit for -', self.symbol, 'is -->', len(stopLossList))
                dataconfig.exit_position(ExitId)
                ...
                while intrade:
                    ...
                    if time.strftime('%H:%M') == '15:12':
                        ...
                        print('Trade is Squared OFF')
                        return
                    print('Stop_loss2 Value is - ', config1['stop_loss2'])
                    print('Target2 value is - ', config1['target2'])
                    if (side == -1) and (str(ltp) >= config1['stop_loss2']):
                        # ExitId = {"id": "NSE:AARTIIND-EQ-INTRADAY"}
                        print('exit data', ExitId)
                        dataconfig.exit_position(ExitId)
                        ...
                        stopLossList.append(Stoploss_Count + 1)
                        print('Numbers of times StopLoss hit for -', self.symbol, 'is -->', len(stopLossList))
                        print('stopLoss count in config - ', config1['stopLoss_count'])
                        if str(len(stopLossList)) == config1['stopLoss_count']:
                            print('Stop the Trade ')
                            return
                        else:
                            dataconfig.place_order_2()

                    elif (side == -1) and (str(ltp) <= config1['target2']):
                        dataconfig.exit_position(ExitId)
                        print("Profit booked in second go for " + self.symbol + ' at ' + str(ltp))
                        return
            elif (side == -1) and (str(ltp) <= config1['target1']):
    ...
    def place_order_2(self):
        ...
        try:
            ...
            while Flag:
                ...
                print('current time--', time.strftime('%H:%M:%S'))
                time.sleep(1)
                config4 = json.load(open('Sell_config.json'))
                print('Order still not placed in place_order2 for ', self.symbol, 'with LTP -', ltp,
                      ' and with LIMIT Price2 -',
                      config4['limitPrice2'])
                if str(ltp) >= config4['limitPrice2']:
                    response = fyers.place_order(data2)
                    # print('Order Placed  in place_order 2 with LTP ', ltp)
                    print('Order Placed  in place_order 2 with LTP ', ltp,response)
                    return
                else:
                    print('---------------')
...
if __name__ == '__main__':
    dataconfig = Sell_Order()
    StopLossList = []
    dataconfig.generate_token()
    dataconfig.place_order_1(StopLossList)
