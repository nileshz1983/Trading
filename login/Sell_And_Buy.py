...
class Buy_Order:
    def __init__(self) -> None:
        ...
        try:
            ...
            if not os.path.join(os.getcwd().join('/Buy_And_Sell_config.json'.strip())):
        ...
        config = json.load(open('Buy_And_Sell_config.json'))
        print('config--', config)
        ...
        if self.username == '' or self.client_id == '' or self.secret_key == '' or self.totp_key == '' or self.redirect_url == '' or self.target1 == '' or self.target2 == '' or self.stop_loss1 == '' or self.stop_loss2 == '' or self.quantity1 == '' or self.quantity2 == '':
            ...
            print('please enter all the details in the Buy_And_Sell_config.json file')
            exit()

    def create_config_file(self):
        file = json.dumps(config, indent=10)
        with open("../Buy_And_Sell_config.json", "w") as f:
            f.write(file)
            f.close()
    ...
    def place_order_1(self):
        ...
        while intrade == 1:
            ...
            if (side == 1) and (str(ltp) <= self.stop_loss1):
                ...
                while intrade == 0:
                    ...
                    print('count', count + 1)
                    ExitId = {"id": "" + (self.symbol + '-' + self.productType) + ''}
                if (dir == "SELL") and (str(ltp) >= self.stop_loss2):
                    ...
                    intrade = 2
                    return
                elif (dir == "SELL") and (str(ltp) <= self.target2):
                    dataconfig.exit_position(ExitId)
