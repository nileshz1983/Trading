...
class FinNifty_Buy_PE_Option:
    def __init__(self) -> None:
        global config
        try:
            if not os.path.join(os.getcwd().join(Constant.NIFTY_BUY_PE_OPTION.strip())):
        ...
        config = json.load(open(Constant.NIFTY_BUY_PE_OPTION))
        ...
        if self.target1 == '' or self.target2 == '' or self.stop_loss1 == '' or self.stop_loss2 == '' or self.quantity1 == '' or self.quantity2 == '' or self.stop_loss_percent == '' or self.Premium_Price == '':
            ...
            print('please enter all the details in the FinNifty_Buy_PE_Option.json file')
            exit()

    def create_config_file(self):
        file = json.dumps(config, indent=10)
        with open("../../config/FinNifty_Buy_PE_Option.json", "w") as f:
            f.write(file)
            f.close()
    ...
    def place_order_1(self):
        ...
        try:
            ...
            while True:
                ...
                with open(Constant.NIFTY_BUY_PE_OPTION, 'w') as f:
                    f.write(json.dumps(self.config))
                    f.close()
                config4 = json.load(open(Constant.NIFTY_BUY_PE_OPTION))
            ...
            while intrade == 1:
                ...
                config1 = json.load(open(Constant.NIFTY_BUY_PE_OPTION))
...
if __name__ == '__main__':
    dataconfig = FinNifty_Buy_PE_Option()
