...
class BankNifty_Sell_PE_Option:

    def __init__(self) -> None:
        global config

        try:
            if not os.path.join(os.getcwd().join(Constant.BANKNIFTY_SELL_PE_OPTION.strip())):
        ...
        config = json.load(open(Constant.BANKNIFTY_SELL_PE_OPTION))
        # threading.Thread(target=Sell_Order.fileWatcher).start()
        print("\033[96mSell_config--", config)
    ...
    def place_order_1(self):
        global counter, Sell_FinalSL, symbol2

        fyerUtils = Fyers_Utilty()
        classname = str(dataconfig.__class__.__name__)
        ...
        try:
            while True:
                ...
                with open(Constant.BANKNIFTY_SELL_PE_OPTION, 'w') as f:
                    f.write(json.dumps(self.config))
                    f.close()
                config4 = json.load(open(Constant.BANKNIFTY_SELL_PE_OPTION))
        ...
        while intrade == 1:
            ...
            config1 = json.load(open(Constant.BANKNIFTY_SELL_PE_OPTION))
            ...
            if sell_trailing_stop_loss_price < Sell_FinalSL:
                ...
                # print('Sell_FinalSL222--', Sell_FinalSL)
                # print('sell_trailing_stop_loss_price222--', sell_trailing_stop_loss_price)
            if (side == -1) and (ltp is not None and ltp >= Sell_FinalSL):
                ...
                if Order2_Execution_Flag == 'True':
                    BankNifty_Sell_PE_Option.place_order_2(self)
                else:
                    print('-------------------------------------------------------------------------------')
                ...
                while intrade == 0:
                    ...
                    print('Sell_FinalSL111--', Sell_FinalSL1)
                    print('sell_trailing_stop_loss_price1111--', sell_trailing_stop_loss_price1)
                    time.sleep(2)
                    ExitId = {"id": "" + (self.symbol + '-' + self.productType) + ''}
                    if time.strftime('%H:%M') == '15:13':
                        fyerUtils.exit_position(ExitId)
    ...
    def place_order_2(self):
        ...
        try:
            fyers = fyersModel.FyersModel(client_id=fyerUtils.client_id, token=fyerUtils.access_token, log_path="")
            while True:
                filterStock = fyerUtils.get_CE_FNO_Data(classname)

                self.symbol = filterStock
                print('self.symbol IN place_order_2-------', self.symbol)
                symbol3 = {"symbols": self.symbol}
                filterStockPrice = dataconfig.get_LTP(symbol3)
                print(filterStock, dataconfig.get_LTP(symbol3))

                # config4 = json.load(open('Nifty_Sell_CE_Option.json'))
                print('\033[92m-------------------------place_order 2---------------------------')
...
if __name__ == '__main__':
    dataconfig = BankNifty_Sell_PE_Option()
    time.sleep(2)
