...
class Nifty_Hedge_CE_Option:
    def __init__(self) -> None:
        global config
        try:
            if not os.path.join(os.getcwd().join(Constant.NIFTY_HEDGE_CE_OPTION.strip())):
        ...
        config = json.load(open(Constant.NIFTY_HEDGE_CE_OPTION))
        ...
        if self.target1 == '' or self.quantity1 == '' or self.stop_loss_percent == '':
            ...
            print('please enter all the details in the NIFTY_HEDGE_CE_OPTION.json file')
            exit()

    def create_config_file(self):
        file = json.dumps(config, indent=10)
        with open("../../config/NIFTY_HEDGE_CE_OPTION.json", "w") as f:
            f.write(file)
            f.close()
    ...
    def place_order_1(self):
        ...
        try:
            ...
            while True:
                ...
                with open(Constant.NIFTY_HEDGE_CE_OPTION, 'w') as f:
                    f.write(json.dumps(self.config))
                    f.close()
                config4 = json.load(open(Constant.NIFTY_HEDGE_CE_OPTION))
            ...
            while intrade == 1:
                ...
                config1 = json.load(open(Constant.NIFTY_HEDGE_CE_OPTION))
...
if __name__ == '__main__':
    dataconfig = Nifty_Hedge_CE_Option()
